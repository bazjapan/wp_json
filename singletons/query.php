    ...
    if ($json == 'submit_comment') {
      if ($json_api->controller_is_active('respond')) {
        return 'respond';
      	        ...
      	        $json_api->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > Unchained API.");
       }
    } else if ($json == 'create_post') {
      if ($json_api->controller_is_active('posts')) {
        return 'posts';
      } else {
        $json_api->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > Unchained API.");
      }
    } else {
      return 'core';
    }
